...
def clearFields():
    ...
    description.set('');
    payee.set('');
    amount.set(0.0), dateField.set_date(todayDate)
...
def removeExpense():
    ...
    confirmation = mb.askyesno('Είστε σίγουροι;',
                               f'Είστε βέβαιοι ότι θέλετε να διαγράψετε την εγγραφή του {valuesSelected[2]}')
    ...
    if confirmation:
        ...
        mb.showinfo('Η εγγραφή διαγράφηκε με επιτυχία!',
                    'Η εγγραφή που θέλατε να διαγράψετε έχει διαγραφεί με επιτυχία')
...
def addAnotherExpense():
    ...
    if not dateField.get() or not payee.get() or not description.get() or not amount.get() or not modeOfPayment.get():
        mb.showerror('Τα πεδία είναι άδεια!',
                     "Παρακαλώ συμπληρώστε όλα τα πεδία που λείπουν πριν πατήσετε το κουμπί προσθήκης!")
    else:
